...
面板 `[2]` 的字母 hotkey(皆列在 `Space` 選單裡):`C` carry、`y` yank(複製 full path 到剪貼簿)、`e` edit(文字檔在內嵌 `$EDITOR` 編輯,非文字走系統開啟)、`p` 貼上落地、`m` 搬移落地、`P` pin、`R` rename、`a` add、`D` delete、`S` sort、`.` 顯示隱藏檔、`z` zoom。
