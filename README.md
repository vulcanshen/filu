...
- **Breadcrumb path row** — the tab's full path renders as plain lavender text on panel `[1]`'s first row, with a recessive rule below to set it off from the list. A long path shrinks — front segments to their initial (`~/Documents/x` → `~/D/x`), then the middle collapses to `…` — so it never wraps.
- **Marks copy & move** — deferred cp/mv, like Finder's Cmd+C / Cmd+V. `m` marks a file into the marks bucket (a glyph marks it in the list — an in-place multi-select), then you navigate to the target directory and `c` copies / `v` moves the whole bucket there. In the Marks tab, `p` picks a *subset* to land instead of the whole bucket, `m` unmarks an item, and `C` clears the whole bucket (after a confirm — the files are untouched). Copy leaves the bucket intact so you can land to several directories; move updates the bucket paths so they stay valid.

- **Zip the picks** — `Z` in the Marks tab packs whatever a Copy would land (the picked subset, or the whole bucket) into one archive. It prompts for a name, pre-filled from what you picked — the item's own name, the directory they came from, or a timestamp when they span directories. The archive is written to a temp directory, so no working directory is touched, and it joins the bucket as the *only* pick: navigate to where it belongs and `c` / `v` lands it. Packing runs in the background and reports in the Tasks tab. Each pick sits in the archive under its own name (a directory keeps its inner structure); two picks that share a name are both kept.
...
- **Native streaming finders** — a filu-drawn split picker (list + preview), not the fzf binary. Every mode streams its listing (`fd`-order, first results near-instant, filter while it loads):
  - **`/` Search** — opens a chooser: **filename** (fuzzy-match names in the current tab's subtree; a query starting with `/` — or `~/` for your home — re-anchors onto an absolute path instead, so anywhere on disk is reachable) or **content** (filter by `ripgrep`; the preview scrolls to the matched line and highlights it).
  - **`go` Goto** — a picker for a **Favorite** directory, or a fuzzy **Search** of directory paths under `$HOME` (hidden directories included — the `ignore_dirs` blocklist still filters tool dirs like `.git`); a query starting with `/`, or `~/` for a home path, re-anchors onto that absolute path — fuzzy across the whole path, bounded a few levels below the deepest directory it names — so directories outside home are reachable too. `Enter` teleports the active tab there.
...
- **Dynamic directory tabs** — `[1]` opens with one tab; `t` opens a new tab via a `{Same, Favorites, Search}` picker (up to five total), `w` closes the active one; reaching the limit toasts instead of silently doing nothing. The first tab carries the launch glyph (it always opens at the launch directory — the reference the cd-on-quit picker returns to), the rest one animal mark each (cat, dog, paw, easter egg) — the path lives in the panel's breadcrumb, so the tab bar just marks position and which is active.
