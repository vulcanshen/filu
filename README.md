...
- **Native streaming finders** — a filu-drawn split picker (list + preview), not the fzf binary. Every mode streams its listing (`fd`-order, first results near-instant, filter while it loads):
  - **`/` Search** — opens a chooser: **filename** (fuzzy-match names in the current tab's subtree) or **content** (filter by `ripgrep`; the preview scrolls to the matched line and highlights it).
  - **`go` Goto** — a picker for a **Favorite** directory, or a fuzzy **Search** of directory paths under `$HOME`; `Enter` teleports the active tab there.
