...
- **Powerline breadcrumb** — the tab's full path renders as a powerline breadcrumb on panel `[1]`'s first row, coloured along a `crust → blue` gradient by directory depth (root is the darkest chip, where you are is blue). It shrinks to the panel width — front segments to their initial (`~/Documents/x` → `~/D/x`), then the middle collapses to `…` — so it never wraps.
